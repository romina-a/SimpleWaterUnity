...
public class WobbleImproved : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // velocity
        Vector3 new_velocity = (lastPos - transform.position) / Time.deltaTime;
		Vector3 acceleration = (new_velocity - lastVelocity) / Time.deltaTime;
        Vector3 new_angularVelocity = transform.rotation.eulerAngles - lastRot;
		Vector3 angularAcceleration = new_angularVelocity - lastAngularVelocity;


        // add clamped velocity to wobble
        Debug.Log("setting wobble");
        wobbleAmountToAddX += Mathf.Clamp((lastVelocity.x + (lastAngularVelocity.z * 0.2f)) /* * MaxWobble */, -MaxWobble, MaxWobble);
        wobbleAmountToAddZ += Mathf.Clamp((lastVelocity.z + (lastAngularVelocity.x * 0.2f)) /* * MaxWobble */, -MaxWobble, MaxWobble);

        // keep last position
        lastPos = transform.position;
        lastRot = transform.rotation.eulerAngles;
        lastVelocity = new_velocity;
        lastAngularVelocity = new_angularVelocity;
    }
