Shader "Romina/CylinderWaterShader"
{
    Properties
    {
    ...
    SubShader
    {
		...
		************************************/
		...
		float get_ellipse_value(float4 newvertex)
		{
		...
		{
			if (newvertex.y>_Scale.y || newvertex.y<-_Scale.y)
			{
				return false;
            }
			float x = get_ellipse_value(newvertex);
			return x<=1;
        }
        ...
        v2f vert (appdata v)
        {
			...
			if (pic >= 0)
			{
				...
				if (is_inside(newvertex) == false){
					newvertex = float4(newvertex.xyz/sqrt(get_ellipse_value(newvertex)),1);
					o.fillEdge = -100;
				}
				else
				{
